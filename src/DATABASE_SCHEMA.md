...
A log of content changes submitted by HR for Admin review and a log of AI agent actions for auditing.

| Column             | Type      | Description                                                                                         |
| ------------------ | --------- | --------------------------------------------------------------------------------------------------- |
| `id`               | `UUID`    | **Primary Key**.                                                                                    |
| `company_id`       | `UUID`    | **Foreign Key** to `companies.id`. The company this review item belongs to.                         |
| `user_email`       | `TEXT`    | The email of the HR user who submitted the item, or the end-user for an AI audit.                   |
| `type`             | `TEXT`    | The type of review item (`custom_question_guidance`, `question_edit_suggestion`, or `ai_recommendation_audit`). |
| `status`           | `TEXT`    | The current status ('pending', 'approved', 'rejected', 'withdrawn', 'reviewed').                    |
| `change_details`   | `JSONB`   | For HR suggestions, a structured object with the details of the suggested change.                   |
| `input_data`       | `JSONB`   | For AI audits, the user profile and assessment data that was fed to the AI agent.                   |
| `output_data`      | `JSONB`   | For AI audits, the structured recommendation object that was generated by the AI agent.             |
